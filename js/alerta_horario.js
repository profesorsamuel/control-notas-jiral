...
// =========================================================
// CONFIGURACIÓN DEL SONIDO (cámbialo aquí a tu gusto)
// =========================================================
// TIPO: "sine" (suave), "square" (más fuerte/electrónico),
//       "triangle" (intermedio), "sawtooth" (áspero)
// FRECUENCIA: número más alto = sonido más agudo (prueba 440–1200)
// VECES: cuántos pitidos seguidos suenan por alerta
const CONFIG_SONIDO = {
    tipo: "sine",
    frecuencia: 880,
    veces: 3,
...
// Activa o desactiva que además de sonar, una voz LEA el aviso
// (ej. "Ciencias Naturales empieza en 5 minutos")
const LEER_EN_VOZ_ALTA = true;
...
const btnEscucharProxima = document.getElementById("btnEscucharProxima");
if (btnEscucharProxima) btnEscucharProxima.addEventListener("click", leerProximaClase);
...
function pitido(vecesRestantes = CONFIG_SONIDO.veces) {
    if (!audioCtx) return;
    const osc = audioCtx.createOscillator();
    const gain = audioCtx.createGain();
    osc.type = CONFIG_SONIDO.tipo;
    osc.frequency.value = CONFIG_SONIDO.frecuencia;
...
// =========================================================
// VOZ (Web Speech API, gratis, viene en el navegador)
// =========================================================

function hablar(texto) {
    if (typeof window.speechSynthesis === "undefined") return;
    try {
        window.speechSynthesis.cancel(); // no se amontonan avisos
        ...
        console.warn("No se pudo leer en voz alta:", e);
    }
}

function proximaClase() {
    const ahora = segundosAhora();
    return clasesHoy.find(c => c.finSeg > ahora) || null;
}

function leerProximaClase() {
    const c = proximaClase();
    if (!c) {
        hablar("No tienes más clases por hoy.");
        return;
    }
    const ahora = segundosAhora();
    const enCurso = ahora >= c.inicioSeg && ahora < c.finSeg;
    const texto = enCurso
        ? `Ahora mismo: ${c.titulo}. Termina a las ${formatearHora12(c.horaFinTexto)}.`
        : `Tu próxima clase es ${c.titulo}, a las ${formatearHora12(c.horaInicioTexto)}.`;
    hablar(texto);
}
...
function dispararAlerta(texto) {
    ...
    if (LEER_EN_VOZ_ALTA) hablar(texto);
}
