// =========================================================
// Panel compartido del docente para los exámenes de recuperación.
// =========================================================
...
async function verificarAcceso() {
  const estado = document.getElementById("admin-login-estado");
  ...
  const esDocente = Array.isArray(asignaciones.data) && asignaciones.data.length > 0;
  // Una política RLS puede ocultar una de las dos tablas. Basta con que
  // una fuente confirme el rol; solo tratamos los errores como bloqueo
  // cuando ninguna fuente pudo autorizar la cuenta.
  if (!esAdmin && !esDocente && (perfil.error || asignaciones.error)) {
    console.error("No se pudo verificar el acceso:", perfil.error || asignaciones.error);
    estado.textContent = "No se pudo verificar el acceso. Intenta iniciar sesión nuevamente.";
    estado.className = "error";
    return;
  }
  if (!esAdmin && !esDocente) {
    estado.textContent = "Esta cuenta no tiene permiso de docente o administrador.";
    estado.className = "error";
    return;
  }
...
document.getElementById("btn-vista-previa")?.addEventListener("click", () => {
  window.open(`${CONFIG.paginaExamen}?preview=1`, "_blank", "noopener");
});
