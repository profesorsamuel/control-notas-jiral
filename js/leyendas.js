...
// vez por salón + materia (aplica a todos los trimestres de ese
// salón/materia, pero NO a los demás salones que compartan la misma
// materia) y se muestra automáticamente en la pantalla de Consulta
// de notas del estudiante, debajo de la tabla de esa materia.
...
export async function obtenerLeyendaMateria(salon, materia) {
    ...
    if (error) {
        console.error("No se pudo leer la leyenda del salón/materia:", error);
        return "";
    }
    return data?.texto || "";
}
...
// upsert por (salon, materia) (clave primaria de la tabla), así que
...
export async function guardarLeyendaMateria(salon, materia, texto) {
    ...
    if (!limpio) {
        const { error } = await supabase.from("leyendas_materia").delete()
            .eq("salon", salon).eq("materia", materia);
...
// por cada una). Usa clavesLeyenda(salon, materia) para armar/leer la
// misma clave desde otros archivos.
export function claveLeyenda(salon, materia) {
    return `${salon}||${materia}`;
}
...
export async function obtenerTodasLasLeyendas() {
    ...
    const mapa = {};
    (data || []).forEach((fila) => { mapa[claveLeyenda(fila.salon, fila.materia)] = fila.texto; });
    return mapa;
}
