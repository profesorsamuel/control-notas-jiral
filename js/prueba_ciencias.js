// =========================================================
// Motor compartido para todos los exámenes de recuperación.
// C.E.B.G. El Jiral
// =========================================================
...
if (!Array.isArray(BANCO)) {
  throw new Error(`No se encontró el banco configurado: ${CONFIG.bancoGlobal}`);
}
...
async function finalizarQuiz() {
  ...
  if (quizState.modo === "oficial" && !MODO_VISTA_PREVIA) {
    ...
    const { error: errGuardarPractica } = await sb.from(T.intentosPractica).insert({
      ...
      nombre: estudiante.nombre,
      salon: estudiante.salon,
      preguntas_ids: quizState.preguntasIds,
      respuestas,
      correctas, incorrectas, porcentaje, nota_meduca: nota,
      tiempo_total_seg: tiempoTotal,
