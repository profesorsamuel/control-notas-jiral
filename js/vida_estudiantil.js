...
async function subirFoto(card,id){
  const blob=seleccion.get(id);if(!blob)return;const btn=card.querySelector('.guardar');try{const salon=exigirEstudiante();btn.disabled=true;btn.textContent='Subiendo…';const path=`${usuario.id}/${estudiante.id}/fotos/${id}.webp`;const {error:up}=await supabase.storage.from(BUCKET).upload(path,blob,{contentType:'image/webp',upsert:true});if(up)throw up;
    ...
    const {data,error}=await supabase.from('vida_estudiantil_recuerdos').upsert(fila,{onConflict:'usuario_id,estudiante_id,categoria'}).select().single();if(error)throw error;archivos.set(id,data);seleccion.delete(id);await pintarArchivo(card,data);actualizarProgreso();mensaje('Fotografía subida y agregada a tu collage.','ok');btn.hidden=true;
