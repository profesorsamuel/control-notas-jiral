...
async function descargarCollage(){
  const salon=exigirEstudiante(),btn=$('descargarCollage');btn.disabled=true;btn.textContent='Creando collage…';try{const c=document.createElement('canvas');c.width=2400;c.height=1500;const x=c.getContext('2d');const grad=x.createLinearGradient(0,0,2400,0);grad.addColorStop(0,'#073b67');grad.addColorStop(.72,'#0b5c99');grad.addColorStop(1,'#c1121f');x.fillStyle=grad;x.fillRect(0,0,2400,210);x.fillStyle='#fff';x.textAlign='center';x.font='bold 72px Arial';x.fillText('ESTUDIANTES GRADUANDOS 2026',1200,92);x.font='bold 42px Arial';x.fillText(`C.E.B.G. EL JIRAL · ${estudiante.nombre} · ${salon.replace('9','9.º ')}`,1200,158);
