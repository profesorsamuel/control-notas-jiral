...
async function subirFoto(card,id){
  const blob=seleccion.get(id);if(!blob)return;const btn=card.querySelector('.guardar');try{const salon=exigirEstudiante();btn.disabled=true;btn.textContent='Subiendo…';const path=`${usuario.id}/fotos/${id}.webp`;const {error:up}=await supabase.storage.from(BUCKET).upload(path,blob,{contentType:'image/webp',upsert:true});if(up)throw up;
    ...
    const existente=archivos.get(id);let consulta;if(existente){consulta=supabase.from('vida_estudiantil_recuerdos').update(fila).eq('id',existente.id)}else{consulta=supabase.from('vida_estudiantil_recuerdos').insert(fila)}const {data,error}=await consulta.select().single();if(error)throw error;archivos.set(id,data);seleccion.delete(id);await pintarArchivo(card,data);actualizarProgreso();mensaje('Fotografía subida y agregada a tu collage.','ok');btn.hidden=true;
