...
import { obtenerTodasLasLeyendas, claveLeyenda } from "./leyendas.js";

// Texto explicativo por salón + materia (ej. "Nota 1 es de la clase
// 1...") que el/la docente escribe desde su panel. Se carga una vez
// por consulta y se reutiliza en cada render() (cambiar de trimestre
// no cambia el texto, así que no hace falta pedirlo de nuevo cada
// vez). Se muestra la del salón ACTUAL del estudiante.
    ...
    resumen.forEach(({ materia, datos, promApr, promEje, promExa, promFinal, fracaso }) => {
        ...
        html += `
            <div class="${claseTarjeta}">
                ...
                ` : ""}
                ${leyendasPorMateria[claveLeyenda(estudianteActual?.salon, materia)] ? `
                    <p class="aviso-leyenda-card">
                        ℹ️ ${escapeHtml(leyendasPorMateria[claveLeyenda(estudianteActual?.salon, materia)]).replace(/\n/g, "<br>")}
                    </p>
                ` : ""}
            </div>
        `;
